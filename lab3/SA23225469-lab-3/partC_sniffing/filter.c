...
bool filterByPort(unsigned char *buffer, unsigned short port, int type){
    // Exercise 3: complete filter by MAC address, protocol type, and port number:
    // Add your code here:
    // 默认传入的参数是经过filterByProtocol过滤的，只会有TCP/UDP Packet
    struct ethhdr *eth = (struct ethhdr*)buffer;
    if((int)ntohs(eth->h_proto) != ETHERTYPE_IP) {
        return false;
    }
    ...
    switch (ip->protocol) {
        case IPPROTO_TCP:{
            ...
            if (type == 0 && port == ntohs(tcp->th_sport)) {
                return true;
            }
            else if (type == 1 && port == ntohs(tcp->th_dport)) {
                return true;
            }
            else
                return false;
        ...
        case IPPROTO_UDP:{
            ...
            if (type == 0 && port == ntohs(udp->uh_sport)) {
                return true;
            }
            else if (type == 1 && port == ntohs(udp->uh_dport)) {
                return true;
            }
            else 
                return false;
