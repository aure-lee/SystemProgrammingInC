...
int main (int argc, char **argv){
    ...
    while(1){
        ...
        if(n > 0){
            if(filterByProtocol((unsigned char *)buf, 4)) {
                // loops ++;
                packet_process((unsigned char *)buf, n);
                // 使用PartA实现的square-server-udp和square-client-udp
                if (filterByPort((unsigned char *)buf, 12345, 0))
                    packet_process((unsigned char *)buf, n);
            }
        }else{
